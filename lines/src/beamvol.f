***********************
      SUBROUTINE BEAMVOL (ERROR)
***********************
*     beam volume (beam pattern integral) calculation for a drift scan
*
*
*     CMDP command parameters used:
*     1 = command mnemonic BV
*     2 = X value of beam start (usually J2000 RA)
*     3 = X value of beam centre (usually J2000 RA)
*     4 = X value of beam end (usually J2000 RA)
...
*
      ...
      REAL*8    BC                  ! beam centre in X units (degrees)
      REAL*8    BE                  ! beam end in X units (degrees)
      REAL*8    BS                  ! beam start in X units (degrees)
*
      INCLUDE 'lines.inc'
*
*
...
*
      BS = -1111D0
      IF (NCMD .GE. 2) THEN
          READ (CMDP(2),*,IOSTAT=IOS,ERR=60) BS
   60     IF (IOS .NE. 0) THEN
              PRINT 2005, CMDP(2)
              PRINT '(A,$)',' beam start? '
              READ '(A)', CMDP(2)
              READ (CMDP(2),*,IOSTAT=IOS,ERR=61) BS
   61         IF (IOS .NE. 0) THEN
                  PRINT 2005, CMDP(2)
                  ERROR = 1
               GO TO 900
...
*
      BC = -1111D0
      IF (NCMD .GE. 3) THEN
          READ (CMDP(3),*,IOSTAT=IOS,ERR=70) BC
   70     IF (IOS .NE. 0) THEN
              PRINT 2005, CMDP(3)
              PRINT '(A,$)',' beam centre? '
              READ '(A)', CMDP(3)
              READ (CMDP(3),*,IOSTAT=IOS,ERR=71) BC
   71         IF (IOS .NE. 0) THEN
                  PRINT 2005, CMDP(3)
                  ERROR = 1
               GO TO 900
...
*
      BE = -1111D0
      IF (NCMD .GE. 4) THEN
          READ (CMDP(4),*,IOSTAT=IOS,ERR=80) BE
   80     IF (IOS .NE. 0) THEN
              PRINT 2005, CMDP(4)
              PRINT '(A,$)',' beam end? '
              READ '(A)', CMDP(4)
              READ (CMDP(4),*,IOSTAT=IOS,ERR=81) BC
   81         IF (IOS .NE. 0) THEN
                  PRINT 2005, CMDP(4)
                  ERROR = 1
               GO TO 900
