/* Uses the file "observations.txt" to generate a reduction script for LINES
 *
 * The list of observations is generated as follows:
 *  -Generate a list of all spectra files to be reduced e.g. ls *.asc > list.
 *  -The LINES command 'la' will list the contents of the spectra files and
 *    write the listing in the correct format to the file 'lines.fla'
 *  -Use the program EXTRACT-LOG to generate a listing of sets of observations
 *   with the spectrum numbers of the pointing observations and start and stop spectrum
 *   numbers of the on-source observations.
 *
 * The output file from EXTRACT-LOG (input file for this program) is of the form:
 * Filename,Source_ID,North,South,East,West,Start,Stop,Date,Scan,Tsys
 * where
 * Filename is the name of the file containing the raw spectra
 * SourceID is the name of the source - as specified in the observing source file
 ...
 * Scan is the scan number of the last on-source scan
 * Tsys is the antenna system temperature
 * The LINES reduction script is written to the file 'reductions'
 * Type 'inp reductions' in LINES to run the script
 */
...
{
	FILE *obs_log, *script;
	char *infilename,*name,*north, *south, *east, *west, *start, *stop, *date, *tsys, *scan, *endptr;
	char sep[] = ",";
	...
	/* The following command must only be executed once at the start of the reduction run*/
	fprintf(script, "set plotdev /xw \n\n");

	/* Do the following loop while there are still observations to be processed */
	while (fgets(line, 500, obs_log)!=NULL)
	{
		...
		/* Generate a filename for the reduced spectra.  All observations should be written
		 * to this file in chronological order
		 */
		strcpy(outfilename,"../");
		strcat(outfilename,infilename);

		/* Determine if pointing observations exist
		 */
		if (strcmp(north, "0")==0)
		...
		/* Set the velocity blocks for the baseline fit and peak  velocity for pointing corrections.
		 * This has to be set individually for each source.
		 * For sources where there are no pointing observations done, set pca = ''
		 */
		strcpy(pca, "");
		strcpy(pb, "");

		/* It would be advisable to use the source name as the identifier rather than the infilename if files in multiple directories are used.
		 * The infilename was used for 4.5cm reductions because source names changed slightly at various times when the observing
		...
		if (strcmp(infilename,"sgngc.asc")==0)
		{
			strcpy(pca, "pca -10.45");
		...
		if (strcmp(name,"9.62+0.2            ")==0)
		{
			strcpy(pca, "pca 1.21  ");
			strcpy(pb, "pb  -4.56 -2.20     \n pb 7.25 9.67        ");
		}
		if (strcmp(infilename,"sgw31.asc")==0)
		{
			strcpy(pca, "");
		 ...
		 * This will have to modified depending on the specific reduction path you follow
		 */
  		fprintf(script, "clr all \n");
		fprintf(script, "pb clear \n");

		/* Write the commands for calculating the pointing corrections */
		if (strcmp(pointing,"TRUE")==0)
		{
			fprintf(script, "do i %s %s 1 \n",north, start);

			/* The spectra containing the pointing observations have to be written
			 * into the LINES memory starting at buffer 1.  If this set of observations
			 * is not the first in the spectra file, the offset needs to be calculated.
			 */
			if (strcmp(north, "1")!=0;
		...
		/* Now to average the on-source spectra, apply pointing corrections, fit baseline and apply PSS */
