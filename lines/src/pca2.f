      SUBROUTINE PCA2 (ERROR)
********************
* automatically calculate pointing correction from spectra NSEW On
* using gausfit2ds for two-dimensional Gaussian fitting
* compare pca which uses 1D gaussian fitting.
* calls: offset
*
*     CMDP command parameters used:
*     1 = command mnemonic PCA2
...
*
      ...
      REAL*8    UNCERTAINTY     ! uncertainty in intensities for 2d gausfit
...
*     write parameters to CMDP for call to calculate pointing correction
      ...
      WRITE (CMDP(5),*) SW
      WRITE (CMDP(6),*) SP
*     need estimate of uncertainty in amplitude values - use ON data set
*     use rms noise combined with error estimate in system temperature
      UNCERTAINTY = SQRT (RMS(MEMON)**2 
     &    + (DTSYS(MEMON)/TSYS(MEMON)*SP)**2)
      WRITE (CMDP(7),*) UNCERTAINTY
